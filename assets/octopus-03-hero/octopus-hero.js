...
(function () {
  ...
  class OctopusHeroSection extends HTMLElement {
    ...
    _render() {
      ...
      // Scroll tracking — use viewport height so the animation spans the full hero scroll range.
      // Previously used this._H (canvas height ~500px) which caused scrollProg to hit 1
      // while the user was still inside the 100vh hero section.
      const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
      const scrollRange = Math.max(window.innerHeight, this._H);
      const scrollProg = Math.max(0, Math.min(1, scrollTop / scrollRange));
